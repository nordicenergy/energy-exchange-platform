...
describe('metrics', function () {
  ...
  describe('#toScore', function () {
    describe('PR', function () {
      it('returns score dependent on value of "S" metric', function () {
    ...
    describe('MAV', function () {
      it('"X" returns the score for "AV"', function () {
    ...
    describe('MAC', function () {
      it('"X" returns the score for "AC"', function () {
    ...
    describe('MPR', function () {
      it('"X" returns the score for "PR"', function () {
        expect(metrics.toScore.MPR.X(null, { PR: 1 })).to.equal(1);
        expect(metrics.toScore.MPR.X(null, { PR: 0 })).to.equal(0);
      });
      it('returns score for value "L" dependent on value of "MS" falling back to "S" metric', function () {
        expect(metrics.toScore.MPR.L({ MS: 'U', S: 'C' })).to.equal(0.62);
        expect(metrics.toScore.MPR.L({ S: 'C' })).to.equal(0.68);
      });
      it('returns score for value "H" dependent on value of "MS" falling back to "S" metric', function () {
    ...
    describe('MUI', function () {
      it('"X" returns the score for "UI"', function () {
    ...
    describe('MS', function () {
      it('"X" returns the score for "S"', function () {
    ...
    impactMetrics.forEach(function (metric, im) {
      describe('M' + metric, function () {
        impactValues.forEach(function (value, iv) {
          it('"' + value + '" returns the "' + value + '" score from "' + metric + '" times the "' + metric + 'R" score', function () {
            var reqKey = metric + 'R';
            var scores = { };
        ...
        it('"X" returns the score from "' + metric + '" times the "' + metric + 'R" score', function () {
          var reqKey = metric + 'R';
          var scores = {};
